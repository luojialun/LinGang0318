 ...
 * 自定义View，圆形进度条
 */
public class CustomCircleProgressSmall extends ProgressBar{

    //定义一些属性常量
    private static final int PROGRESS_DEFAULT_COLOR = 0xFFd3d6da;//默认圆(边框)的颜色
    private static final int PROGRESS_REACHED_COLOR = 0X1E2B58;//进度条的颜色
    private static final int PROGRESS_REACHED_HEIGHT = 2;//进度条的高度
    private static final int PROGRESS_DEFAULT_HEIGHT = 1;//默认圆的高度
    ...
    public CustomCircleProgressSmall(Context context) {
        this(context,null);
    }

    public CustomCircleProgressSmall(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public CustomCircleProgressSmall(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        //获取自定义属性的值
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CustomCircleProgress);
        //默认圆的颜色
        mDefaultColor = array.getColor(R.styleable.CustomCircleProgress_progress_default_color, Color.parseColor("#979797"));
        //进度条的颜色
        mReachedColor = array.getColor(R.styleable.CustomCircleProgress_progress_reached_color, getResources().getColor(R.color.title_bg));
        //默认圆的高度
