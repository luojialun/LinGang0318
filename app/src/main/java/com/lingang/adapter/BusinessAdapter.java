...
public class BusinessAdapter extends RecycleBaseAdapter<BusinessBean.DataBean.ListBean> {
    public BusinessAdapter(Context context, List<BusinessBean.DataBean.ListBean> data) {
        super(context, data);
    }
    ...
    protected void convert(final BaseViewHolder holder, final BusinessBean.DataBean.ListBean item, int position) {
        ...
        if (item.getProjectState().equals("1")){
            holder.setImageResource(R.id.img_state,R.mipmap.business_weikaishi);
        }else if (item.getProjectState().equals("2")){
            holder.setImageResource(R.id.img_state,R.mipmap.business_jinxingzhong);
        }else if (item.getProjectState().equals("3")){
            holder.setImageResource(R.id.img_state,R.mipmap.business_yijieshu);
        }
