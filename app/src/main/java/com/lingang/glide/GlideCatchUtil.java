...
public class GlideCatchUtil {
    ...
    /**
     * 获取自定义Glide缓存路径磁盘缓存大小
     */

    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(App.getInstance().getCacheDir() + "/" + Constants.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(App.getInstance().getCacheDir() + "/" + Constants.GLIDE_CARCH_DIR, true);
    }
